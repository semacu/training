...
- Click on `IntroR.Rmd.zip`, then press `Download` and save the file in your preferred folder
- Go to the folder where you saved `IntroR.Rmd.zip` and uncompress it, e.g. in Mac just double-click on `IntroR.Rmd.zip`. Only then, the file `IntroR.Rmd` will appear.
...
Work in pairs, meet the person sitting next to you and try the following together:

- Create a sequence of numbers from 10 to 30 spaced by three values
- How about decreasing sequences? Now try from 30 to 10 spaced by three values  (check the function `?seq` for help)
- Round the number `pi` down to 1 decimal place (check the function `?round` for help)
...
A vector can also contain text, however unlike values, these need to be captured using `" "`:
...
You will first need to find the **path** to the file `patient-data-cleaned.csv`, which was downloaded together with the course materials. Use the function `file.choose()` to open a dialogue box and browse through the directories to reach the file. The path will then be displayed in R:

```{r eval=FALSE}
file.choose()
```

e.g. `/Users/martin03/Desktop/IntroR/patient-data-cleaned.csv`. The file `patient-data-cleaned.csv` is a comma-separated values (CSV) file, which can easily be opened using softwares like Excel. In R, use the `read.csv()` function and the path obtained above to create a data frame object:
...
# Viewing contents
head(patient_data)
View(patient_data) # only in RStudio
...
R works such that the values in each column have to all be of the same type (i.e. all numbers or all text).
...
Just like with vectors, a selection of a data frame can be made using square brackes `[ , ]`, however data frames are two-dimensional objects so you'll need both *row* and *column* indexes. 
...
If you'd like to see all rows, or all columns, you can neglect either the row or column index respectively. But ... remember to keep the comma ;)
...
# You can also select only specific columns by column name, e.g. give me only the ID, Name, State and Disease Grade
patient_data[patient_data$Height > 183 & patient_data$Smokes == "Smoker", c("ID", "Name", "State", "Grade")]
```

The useful subsetting operators here are **and** `&`, **or** `|` and **in** `%in%`.
...
- Select patients that have a BMI greater than 25 however their weight is less than 65kg. Calculate their average height.
